...
# Support both:
# - running as a script: `python gsplat/examples/datasets/mesh_from_pointcloud.py ...`
# - running as a module: `python -m datasets.mesh_from_pointcloud ...`
try:
    from .read_write_model import (  # type: ignore
        qvec2rotmat,
        read_images_binary,
        read_images_text,
...
def _points_dict_to_arrays(points_dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for pt in points_dict.values():
        xyz.append(pt.xyz.astype(np.float64))
        rgb.append(pt.rgb.astype(np.float64) / 255.0)
...
def load_point_cloud(path: str) -> o3d.geometry.PointCloud:
    ...
    if ext == ".ply":
        ...
        xyz, rgb = _points_dict_to_arrays(read_points3D_binary(path))
        ...
        xyz, rgb = _points_dict_to_arrays(read_points3D_text(path))
        pcd = o3d.geometry.PointCloud()
        if xyz.size:
            pcd.points = o3d.utility.Vector3dVector(xyz)
            if rgb.size:
...
def load_camera_centers(images_path: str) -> np.ndarray:
    ...
    if ext == ".bin":
        ...
        raise ValueError(f"Unsupported images file: {images_path}")
...
def filter_points_near_cameras(
    ...
    if kept_points.size == 0:
        raise RuntimeError("No points remain after near-camera filtering. Adjust thresholds.")
    ...
    print(f"[mesh] Filtered {len(kept_points)} points near cameras (ratio={len(kept_points)/len(pcd.points):.2f})")

    if voxel_size and voxel_size > 0.0:
        filtered = filtered.voxel_down_sample(voxel_size=voxel_size)
        print(f"[mesh] Voxel down-sampled collision cloud to {len(filtered.points)} points (voxel={voxel_size} m)")
...
def reconstruct_mesh_from_points(
    ...
    *,
    ...
    if points.size == 0:
        raise RuntimeError("Mesh reconstruction received an empty point array.")
    ...
    print(f"[mesh] Voxel grid dims = ({nx}, {ny}, {nz}), padding = {padding:.2f}")
    ...
    if dens_max == 0:
        raise RuntimeError("Density field is empty (all zeros); cannot reconstruct mesh.")
    ...
    print(f"[mesh] Final mesh → {len(mesh.vertices)} verts, {len(mesh.triangles)} faces")
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--point-cloud", required=True, help="Path to point cloud (.ply, points3D.bin, points3D.txt).")
    parser.add_argument("--images", required=True, help="Path to COLMAP images file (.bin or .txt) providing camera poses.")
    parser.add_argument("--output-mesh", required=True, help="Destination mesh path (e.g. meshes/scene.obj).")

    parser.add_argument("--every-k", type=int, default=1, help="Uniform point-cloud down-sample factor (<=1 disables).")
    parser.add_argument("--filter-horiz", type=float, default=4.0, help="Horizontal padding (m) around camera centers.")
    parser.add_argument("--filter-vert", type=float, default=1.5, help="Vertical padding (m) around camera centers.")
    parser.add_argument("--filter-voxel", type=float, default=0.0, help="Voxel size (m) for collision cloud down-sample.")
    ...
    parser.add_argument("--padding", type=float, default=0.10, help="Bounding-box padding (m) applied before voxelization.")
    parser.add_argument("--iso-level", type=float, default=None, help="Marching cubes iso-level (defaults to density mean).")
    parser.add_argument(
        "--gaussian-sigma",
        type=float,
        nargs=3,
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    reconstruct_mesh_from_points(
        ...
        gaussian_sigma=tuple(args.gaussian_sigma) if args.gaussian_sigma else None,
